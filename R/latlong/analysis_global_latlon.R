#### Analysis Global lat Lon ####

# Read, prepare and calculate statistics for all tiles at all locations
# Calculates the difference to it's original prediction in absolute and relative values, also SD and correlation
# Mean of pixelwise differences to original

packages <- c("terra", "stringr", "dplyr", "sf", "tibble", "tmap", "ggplot2", "purr", "rnaturalearth", "progressr")
...
latlon_global_process <- function(directories){
  ...
  results <- list()
    ...
    for (i in seq_along(directories)) {
      ...
      pb <- txtProgressBar(min = 0, max = length(tifs), style = 3) # Include comparison to origonal. If not length-1
...
# Deploy with timing
start_time <- Sys.time()
summary_global <- latlon_global_process(tile_dirs)
end_time <- Sys.time()
