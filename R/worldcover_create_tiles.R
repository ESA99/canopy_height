...
worldcover_dir <- "/home/emilio/canopy_height/deploy_example/ESAworldcover/2020/new/orig_zip/WORLDCOVER/"
output_dir <-     "/home/emilio/canopy_height/deploy_example/ESAworldcover/2020/new/WC_merged_tiles"
...
for (i in sentinel_epsg$tile_code) {
  ...
  if (sentinel_epsg[sentinel_epsg$tile_code == TILENUM,]$WC_N == "") {
    cat("Skipping", i, "| Just renaming..\n")
    
    tryCatch({
    in_file <- grep(sentinel_epsg[sentinel_epsg$tile_code == TILENUM,]$WC_N, wc_files, ignore.case = TRUE, value = TRUE)
    out_file <- file.path(output_dir, paste0("WC_",TILENUM,".tif") )
    file.copy(in_file, out_file, overwrite = T)
    }, error = function(e) {
      message("Caught an error while file.copy: ", e$message)
    })
